...
exports.generate_doc = function (filename, outputPath, template, unsafe, autoPages, callback) {
    var markdown = fs.readFileSync(filename).toString();
    write_html(filename, markdown, outputPath, template, unsafe, autoPages, callback);
};

// This function writes parsed output to html
function write_html(filename, markdown, outputPath, template, unsafe, autoPages, callback) {
    var outfile,
        templatePath,
        template;
    ...
    fs.writeFileSync(outfile, fn({ data: marked(markdown, autoPages, unsafe) }));
